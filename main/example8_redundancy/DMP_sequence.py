...
while data.time <= T:
    ...
    if data.time <= t0:
        ...
        if data.time <= t0 + D/2:
            ...
            g = g_new[ :2 ] + ( pf[ :2 ] - g_new[ :2 ] ) * np.exp( -sd.tau * ( data.time - ( t0 + D/2 ) ) )
    ...
    p_arr   =  np.append(   p_arr, 0 )
    dp_arr  =  np.append(  dp_arr, 0 )
    ddp_arr =  np.append( ddp_arr, 0 )

    # Using the Sliding mode control
    dpr  = dp_arr + Gp @ ( p_arr - p )
